@tool
extends Node
class_name TopDownControllerKeyboardComponent2D

@export var velocityComponent: VelocityComponent2D:
	set(value):
...
func updateControl(delta):
	_reset_move()
	var pos = Input.get_vector("look_left","look_right","look_up","look_down")
	if pos != null:
		look_direction = pos 
